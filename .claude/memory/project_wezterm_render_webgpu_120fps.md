...
- On Windows the live config is the **Windows clone** WezTerm reads via `WEZTERM_CONFIG_FILE` — a separate checkout from the WSL repo; a fix in WSL doesn't reach the terminal until that clone is synced. Related: no `wezterm.gui.*` at config-load ([[project_wezterm_no_gui_calls_at_config_load]]).
