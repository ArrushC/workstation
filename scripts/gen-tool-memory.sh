...
emit_block() {
  ...
  printf '<!-- regenerated on config*.toml edits by .claude/hooks/sync-tool-memory.sh; run `cza` to deploy -->\n'
...
# python-env, nerd-fonts, dozzle, vcpkg (the [vars] declaration order), then
...
pins = [
    ...
    ("dozzle", pin_vars.get("dozzle_version", "?")),
