...
# multi-host updates, and bootstrap.ps1 regenerates BOTH managed terminals'
# launch surfaces from it on every run — Invoke-WindowsTerminalFragments
# (the workstation Windows Terminal SSH-profile fragment) and
# Invoke-WarpTabConfigs (the workstation-*.toml Warp Tab Configs). No
# separate inventory file is generated.
#
...
# bootstrap.ps1 regenerates both Windows terminals' SSH launchers from
# hosts.conf on every run — Invoke-WarpTabConfigs (Warp Tab Configs) and
# Invoke-WindowsTerminalFragments (the workstation Windows Terminal
# fragment). PARITY: manage-hosts.sh prints the same note.
function Show-TerminalRefreshNote {
    Write-Log "Warp Tab Configs and Windows Terminal SSH profiles pick this up on the next bootstrap.ps1 run."
}
