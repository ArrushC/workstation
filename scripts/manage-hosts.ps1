...
function Add-Host {
    ...
    Write-Host ""
    $syncAns = Read-Host "  Sync configs now? [Y/n]"
    if (-not $syncAns) { $syncAns = "Y" }
    if ($syncAns -match '^[Yy]$') {
        Invoke-SyncAll
    }
}
