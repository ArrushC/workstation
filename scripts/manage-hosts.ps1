...
# multi-host updates, and bootstrap.ps1's Invoke-WarpTabConfigs regenerates
# the managed Warp Tab Configs (workstation-*.toml) from it on every run.
# No separate inventory file is generated.
#
...
# --- ANSI escape codes (matches manage-hosts.sh; rendered by Warp,
...
# bootstrap.ps1's Invoke-WarpTabConfigs regenerates the managed Warp Tab
# Configs from hosts.conf on every run. PARITY: manage-hosts.sh prints the
# same note.
function Show-WarpRefreshNote {
    Write-Log "Warp Tab Configs (Windows) pick this up on the next bootstrap.ps1 run"
}
