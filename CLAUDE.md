...
- `chezmoi diff` on a host — no surprises. Linux: only Linux-targeted paths (dot_zshrc, dot_bashrc, dot_config/{starship,helix,zellij}, dot_gitconfig, dot_nbrc) + cross-platform `.ssh/config`. Windows: only AppData/Documents/dot_config/wezterm + cross-platform `.ssh/config`.
