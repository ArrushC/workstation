...
- **`chezmoi/private_dot_claude/private_settings.json.tmpl`** — tracked Claude Code user settings; the `statusLine.command` line carries the pinned `ccstatusline@<version>` and MUST mirror `CCSTATUSLINE_VERSION` in `makefile/versions.mk`. Edits apply to `~/.claude/settings.json` on `chezmoi apply` with mode 0600 (parent dir 0700 via the `private_` prefix).
